...
export default function Navbar({ children }) {
  ...
  return (
    <div className="flex h-screen">
      <div
        className={`fixed top-0 left-0 h-full bg-teal-600 shadow-2xl text-white transition-transform transform ${
      ...
      >
        <main className="bg-gradient-to-b from-teal-50 to-teal-100 md:ml-64">
          {children}
        </main>
      </div>

      <div className="md:hidden fixed bottom-0 left-0 w-full bg-teal-600 text-white flex justify-around py-3 rounded-t-lg shadow-lg">
        <Link href="/dashboard">
          <div className="flex flex-col items-center justify-center space-y-1">
            <FiHome
              size={24}
              className="transition-all duration-300 ease-in-out transform hover:scale-110 hover:text-teal-200"
            />
